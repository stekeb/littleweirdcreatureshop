...
// POST /auth/register
router.post("/register", async (req, res) => {           // Endpoint für Registrierung
  const { accountname, password } = req.body || {};      // Body-Felder lesen
  if (!accountname || !password) {                       // Pflichtfelder prüfen
    return res.status(400).json({ message: "accountname und password sind erforderlich" });
  }

  const exists = users.find(u => u.accountname === accountname); // Duplikat prüfen
  if (exists) {                                          // bereits vorhanden → 409
...
// POST /auth/login
router.post("/login", async (req, res) => {              // Endpoint fürs Login
  const { accountname, password } = req.body || {};      // Body-Felder lesen
  if (!accountname || !password) {                       // Pflichtfelder prüfen
    return res.status(400).json({ message: "accountname und password sind erforderlich" });
  }
...
// POST /auth/logout
router.post("/logout", (_req, res) => {                  // Platzhalter ohne Session
