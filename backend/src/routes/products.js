...
// GET /products
router.get("/", (_req, res) => {                    // vollständige Liste
  res.json({ items: products, total: products.length }); // Anzahl und Items zurückgeben
});

router.get("/filter", (req, res) => {               // Filter-Endpunkt
  ...
  if (ability) {                                       // Fähigkeiten (case-insensitive)
    const wanted = String(ability).toLowerCase();
    list = list.filter(p => String(p.ability || "").toLowerCase() === wanted);
  }
  if (minprice && !Number.isNaN(Number(minprice))) { // Mindestpreis in Cents
    list = list.filter(p => Number(p.price) >= Number(minprice));
  }
  if (maxprice && !Number.isNaN(Number(maxprice))) { // Höchstpreis in Cents
